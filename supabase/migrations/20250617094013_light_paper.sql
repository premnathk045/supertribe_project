/*
  # Two-Tier User System Setup

  1. New Tables
    - `profiles` - Extended user profiles with role-based access
    - `creator_verifications` - Creator verification workflow tracking
    - `payment_methods` - Demo payment methods for creators

  2. Security
    - Enable RLS on all tables
    - Add policies for role-based access control
    - Restrict content creation to verified creators
...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO profiles (id, username, display_name, user_type, is_verified)
  VALUES (
    NEW.id,
